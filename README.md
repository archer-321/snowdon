...
- Provide a correct thread-safe implementation of snowflake IDs.<br/>
  The IDs generated by Snowdon should be unique, monotonous, and consistent with other machines in a deployment. You can
...
- Snowflakes are strictly monotonous. If the snowflake `b` was generated *after* the snowflake `a`, `a < b`.
...
monotonous, Snowdon only returns a snowflake `b` after producing the snowflake `a` for all subsequent invocations if
`a < b`.
